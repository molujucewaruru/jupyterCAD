import { DocumentChange, MapChange, StateChange, YDocument } from '@jupyter/ydoc';
...
export type IMainMessage = IDisplayShape | IWorkerInitialized | IDisplayPost | IDryRunResponse;
export interface IWorkerMessageBase {
    id: string;
    action: WorkerAction;
    payload: any;
}
export type IMessageHandler = ((msg: IMainMessageBase) => void) | ((msg: IMainMessageBase) => Promise<void>);
export declare enum JCadWorkerSupportedFormat {
    BREP = "BREP",
    GLTF = "GLTF",
    STL = "STL"
}
...
export interface IJupyterCadDocumentWidget extends IDocumentWidget<SplitPanel, IJupyterCadModel> {
    readonly model: IJupyterCadModel;
}
export interface IJupyterCadOutputWidget extends MainAreaWidget {
    model: IJupyterCadModel;
}
export type IJupyterCadWidget = IJupyterCadDocumentWidget | IJupyterCadOutputWidget;
