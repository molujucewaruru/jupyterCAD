...
interface JCADPlacement {
  ...
  Axis: number[];      // [x, y, z] - rotation axis
...
export class PlacementConverter {
   ...
   */
  static assemblyPositionAxisToJCADPlacement(
    position: number[],
    axis: number[]
  ): JCADPlacement {
   ...
   */
  static assemblyPositionNormalToJCADPlacement(
    position: number[],
    normal: number[]
  ): JCADPlacement {
...
export class GeometryFeatureConverter {
  ...
  static assemblyToJCADAssembly(features: IAssemblyFeature[]): IJCadObject[] {
    ...
    for (const feature of features) {
      const objectName = feature.metadata?.originalObject ||
                        `${feature.type}_${feature.name}`;
      try {
        const jcadObj = AssemblyToJCADConverter.createJCADObject(
          feature,
          objectName
        );
