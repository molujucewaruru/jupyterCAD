...
import { PanelWithToolbar, ToolbarButton, addIcon, homeIcon } from '@jupyterlab/ui-components';
...
export class SuggestionPanel extends PanelWithToolbar {
    constructor(params) {
        ...
        const { model } = params;
        this._model = model;
        const body = ReactWidget.create(React.createElement(Suggestion, { model: model }));
        this.addWidget(body);
        this.addClass('jpcad-sidebar-propertiespanel');
        this.toolbar.addItem('backToRoot', new ToolbarButton({
            icon: homeIcon,
            onClick: async () => {
        ...
        this.toolbar.addItem('newFork', new ToolbarButton({
            icon: addIcon,
    ...
    async createFork() {
        const state = { forkLabel: 'New suggestion' };
        const setForkLabel = value => (state.forkLabel = value);
        const body = ReactWidget.create(React.createElement(NewForkDialog, { setForkLabel: setForkLabel }));
