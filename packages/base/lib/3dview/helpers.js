...
export const DEFAULT_MESH_COLOR = new THREE.Color(getCSSVariableColor(DEFAULT_MESH_COLOR_CSS));
export const DEFAULT_EDGE_COLOR = new THREE.Color(getCSSVariableColor(DEFAULT_EDGE_COLOR_CSS));
export const BOUNDING_BOX_COLOR = new THREE.Color(getCSSVariableColor(BOUNDING_BOX_COLOR_CSS));
export const SPLITVIEW_BACKGROUND_COLOR = new THREE.Color(getCSSVariableColor(SPLITVIEW_BACKGROUND_COLOR_CSS));
export function projectVector(options) {
    const { vector, camera, width, height } = options;
    const copy = new THREE.Vector3().copy(vector);
    copy.project(camera);
    return new THREE.Vector2((0.5 + copy.x / 2) * width, (0.5 - copy.y / 2) * height);
}
export function getQuaternion(jcObject) {
    var _a;
    const placement = (_a = jcObject === null || jcObject === void 0 ? void 0 : jcObject.parameters) === null || _a === void 0 ? void 0 : _a.Placement;
    ...
    return new THREE.Quaternion(axisVector.x * sinHalfAngle, axisVector.y * sinHalfAngle, axisVector.z * sinHalfAngle, Math.cos(halfAngle)).normalize();
}
export function computeExplodedState(options) {
    var _a, _b;
    const { mesh, boundingGroup, factor } = options;
    const center = new THREE.Vector3();
    const meshGroup = mesh.parent;
    ...
    const meshGroupPositionArray = (_b = meshGroup.userData.jcObject.parameters) === null || _b === void 0 ? void 0 : _b.Placement.Position;
    const meshGroupPosition = new THREE.Vector3(meshGroupPositionArray[0], meshGroupPositionArray[1], meshGroupPositionArray[2]);
...
export function buildShape(options) {
    var _a, _b;
    const { objName, data, isSolid, isWireframe, clippingPlanes, objColor } = options;
    const { faceList, edgeList, jcObject } = data;
    const vertices = [];
    const triangles = [];
    const placement = (_b = (_a = data === null || data === void 0 ? void 0 : data.jcObject) === null || _a === void 0 ? void 0 : _a.parameters) === null || _b === void 0 ? void 0 : _b.Placement;
    ...
    for (const face of faceList) {
        ...
        for (let ii = 0; ii < vertexCoorLength; ii += 3) {
            const vertex = new THREE.Vector3(face.vertexCoord[ii], face.vertexCoord[ii + 1], face.vertexCoord[ii + 2]);
    ...
    for (const edge of edgeList) {
        ...
        for (let i = 0; i < edge.vertexCoord.length; i += 3) {
            const vertex = new THREE.Vector3(edge.vertexCoord[i], edge.vertexCoord[i + 1], edge.vertexCoord[i + 2]);
            // Undo placement from the vertices, we want the placement done on the THREE.Object3D (Mesh), not the geometry
            vertex.sub(new THREE.Vector3(objPosition[0], objPosition[1], objPosition[2]));
            vertex.applyQuaternion(inverseQuaternion);
    ...
    const boundingBox = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(size.x, size.y, size.z)), new THREE.LineBasicMaterial({ color: BOUNDING_BOX_COLOR, depthTest: false }));
